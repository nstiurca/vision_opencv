...
TEST(CvBridgeTest, initialization)
{
  ...
  try {
    cv_ptr = cv3_bridge::toCvCopy(image, "mono8");
    // Before the fix, it would never get here, as it would segfault
    EXPECT_EQ(1, 0);
  } catch (cv3_bridge::Exception& e) {
    EXPECT_EQ(1, 1);
  }
...
TEST(CvBridgeTest, imageMessageStep)
{
  ...
  ASSERT_NO_THROW(cv_ptr = cv3_bridge::toCvCopy(image, "mono8"));
  ...
  ASSERT_THROW(cv_ptr = cv3_bridge::toCvCopy(image, "mono8"), cv3_bridge::Exception);
